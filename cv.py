...
# Run
hls_binary = hls_select(image, thresh=(40, 255))

# Plot the result
plot( image, hls_binary, "Thresholded S")
...
# Run
combined_binary, stacked_binary, gradx_binary, grady_binary, mag_binary, dir_binary = threshold( image )
# Define our pipeline
def pipeline(image):

    # Undistort camera image
    undistorted_image = cv2.undistort(image, mtx, dist, mtx)

    # Threshold on the saturation channel
    thresholded_saturation_image = hls_select(undistorted_image, thresh=(40, 255))
    
    # Threshold on sobel gradients
#    thresholded_sobel_image, _, _, _, _, _ = threshold(undistorted_image)
    
    # Perspective warp into bird's-eye view
    output_image = cv2.warpPerspective(thresholded_saturation_image, M, (thresholded_saturation_image.shape[1], thresholded_saturation_image.shape[0]*2) )
    
    # And we're done
    return output_image

# Plot an example pipeline single image result
image = mpimg.imread('test_images/straight_lines1.jpg') 
...
# Plot
plot( image, pipeline(image), "Pipeline Processed" ) 
